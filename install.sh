...
# ─── LLM Setup (Local Default) ──────────────────────────────────────────────

PROVIDER="ollama"
API_KEY=""

info "Defaulting to local LLM (Ollama + Qwen 3.5)"
info "To use Anthropic, MiniMax, or Moonshot, run './thia-lite-config' after install."
...
# Config/Login launcher
cat > "$THIA_DIR/thia-lite-config" << EOF
#!/usr/bin/env bash
source "$THIA_DIR/.venv/bin/activate"
python -m thia_lite config --help
echo ""
echo "Example: ./thia-lite-config llm.provider anthropic --set YOUR_KEY"
EOF
chmod +x "$THIA_DIR/thia-lite-config"
